...
def load_inputs(players_file_obj, merged_file_obj):
    players_name = players_file_obj.name.lower()
    merged_name = merged_file_obj.name.lower()
    ...
    if merged_name.endswith(".csv"):
        merged = pd.read_csv(merged_file_obj)
    elif merged_name.endswith((".xlsx", ".xls")):
        merged = pd.read_excel(merged_file_obj)
    else:
...
def match_players(players_df, merged_df, fallback_projection):
    ...
    out["ProjectedAverage"] = out["MergedAverage"]
...
def build_ranked_players_export(expanded):
    ...
    cols = [
        ...
        "RankLabel", "PosRankNum", "Salary", "ProjectedAverage", "MergedAverage",
        "Total_Games", "Actual_or_Contest_Score", "Form", "Playing Status", "MatchMethod"
    ]
...
if run_button:
    try:
        if players_file is None or merged_file is None:
            st.error("Please upload both the players file and the merged averages file.")
            st.stop()
        ...
        players, merged = load_inputs(players_file, merged_file)
        ...
        merged_df = prepare_merged_df(merged)
        expanded = match_players(players_df, merged_df, fallback_projection=settings["FALLBACK_PROJECTION"])

        unmatched = expanded[expanded["MatchMethod"] == ""]
        if len(unmatched) > 0:
            st.warning(
                f"{len(unmatched.drop_duplicates(subset=['Name']))} players did not match merged averages exactly. "
